...
function Find(HitboxObject,v)
	local Valid = true
	for e,x in HitboxObject.CharStorage do
		if typeof(v) == "table" and x.Part == v or x.Part.Name == v then
			return true
		end
	end
end
function HitboxManager:HookObject(Object:Instance,Type,ID:string,Config)
	Config = Config or nil
	
	local Extensions = Config.Extensions or {}
	local OtherConfig = Config.Config or {}
	ID = ID or ""
	...
	local function Get(HitboxObject:ModuleScript?)
		...
		if HitboxObject then
			...
			for i,v in Extensions do
				if not Find(Mod,v) then
					table.insert(Mod.CharStorage,{
						Part = typeof(v) == "table" and v.Part or v,
						OffsetDir = typeof(v) == "table" and v.OffsetDir or Vector3.zero
					})
				end
			end
			--if Valid then
