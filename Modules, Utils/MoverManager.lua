...
local utilities = Modules:WaitForChild("Utils")
...
if not MoverManager.running then
	...
	local function RunFrame()
		...
		for part,mover:moverConstructor in MoverManager.moverList.Objects do
			if part then
				if not mover.velList then continue end
				if #(mover.velList) == 0 then
					mover.vel.MaxForce = Vector3.zero
				else
					if not mover.velList[1] or typeof(mover.velList[1].val) ~= "Vector3"
 then continue end					mover.vel.Velocity = mover.velList[1].val
				...
				if #(mover.gyroList) == 0 then
					...
					if not mover.gyroList[1] or typeof(mover.gyroList[1].val) ~= "CFrame"
					then continue end
